...
switch ($_GET["op"]) {
    ...
    case 'listar':
		...
		while ($reg=$rspta->fetch_object()) {
			$data[]=array(
            "0"=>'<button class="btn btn-success btn-xs" onclick="agregarDetalle('.$reg->idarticulo.',\''.$reg->nombre.'\','.$reg->precio_venta.')"><span class="fa fa-plus"></span>Añadir</button>',
            "1"=>$reg->nombre,
            "2"=>$reg->categoria,
            "3"=>$reg->codigo,
            "4"=>($reg->stock <= '10')?'<span class="label bg-red">'.$reg->stock.'</span>':'<span class="label bg-green">'.$reg->stock.'</span>',
            "5"=>$reg->precio_venta,
